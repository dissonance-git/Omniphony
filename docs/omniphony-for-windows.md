...
Windows owns the system mix. Omniphony owns endpoint processing. The physical audio driver still owns playback.
...
The 0.1 endpoint APO ingress is still:

```text
stereo float32 → Current → stereo float32
```

So "full Current renderer" means all rendering features available through the current realtime stereo pipeline. It does **not** mean Windows is already passing authored 5.1/7.1/height/object channels directly into Omniphony.

Richer authored multichannel and Windows Spatial Audio/object ingress are later host-layer work. They should extend input truth without replacing the 0.1 endpoint product.
...
After the 0.1 installer is boring and reliable, build upward rather than sideways:

1. make endpoint selection/change handling smoother;
2. improve the tray icon and controls;
3. prove reboot, sleep/resume and device reconnect behavior;
4. add richer authored multichannel ingress where Windows exposes it cleanly;
5. add trustworthy already-binaural detection/bypass when host evidence exists;
6. add optional libaural/research capabilities without replacing the established Current sound.

The 0.1 baseline is intentionally simple: **click install, see the tray icon, and hear Omniphony everywhere through the selected Windows output.**
