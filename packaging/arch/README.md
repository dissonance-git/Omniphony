...
plugin (the `*_bridge.so` pattern), so it is packaged separately (different
repo, different license) and pulled in as a dependency.

## Install layout

```
/usr/lib/liborender.so.0.2.0     # real cdylib (DT_SONAME = liborender.so.0)
/usr/lib/liborender.so.0         # → liborender.so.0.2.0
...
/usr/lib/orender/omniphony_bridge.so  # the decoder bridge plugin
```
...
```
mpv --ad=orender \
    --ad-orender-bridge-path=/usr/lib/orender/omniphony_bridge.so \
...
- `omniphony-bridge` 0.6.0 ← harletty-bridge `v0.6.0`, plus the matching
  Omniphony `liborender-v0.2.0` source for its workspace path-deps
...
```sh
cd omniphony-bridge && makepkg -si
