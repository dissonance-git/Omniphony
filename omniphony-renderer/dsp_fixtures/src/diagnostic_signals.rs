...
mod tests {
    ...
    fn continuous_probes_land_near_one_shared_listening_level() {
        let peak = 0.5f32;
        let scaled_rms = |signal| rms(&run(signal, 240_000)) * peak / level_divisor(signal);
    ...
    fn scaled_bank_is_finite_and_respects_the_requested_peak() {
        for signal in [
            ...
            let measured_peak = samples
                .iter()
                .map(|sample| sample.abs())
                .fold(0.0, f32::max);
            assert!(
                measured_peak <= 0.250_001,
                "{signal:?} exceeded peak: {measured_peak}"
            );
        }
    }
}
