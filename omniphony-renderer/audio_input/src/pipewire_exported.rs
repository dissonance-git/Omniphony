...
impl PipewireBridgeExportNode {
    fn new(
        ...
        let monitor_port_alias_value =
            CString::new(format!("{}:monitor_FL", config.node_description)).expect("valid CString");
        ...
        node.iface.iface.cb.funcs =
            (&PIPEWIRE_BRIDGE_EXPORTED_NODE_METHODS as *const spa::sys::spa_node_methods).cast();
    ...
    fn update_port_params_for_format(&mut self, configured: bool) {
        ...
        self.node_info.change_mask =
            (spa::sys::SPA_NODE_CHANGE_MASK_FLAGS | spa::sys::SPA_NODE_CHANGE_MASK_PARAMS) as u64;
        ...
        self.port_info.change_mask =
            (spa::sys::SPA_PORT_CHANGE_MASK_PROPS | spa::sys::SPA_PORT_CHANGE_MASK_PARAMS) as u64;
        self.port_info.params = self.port_params.as_mut_ptr();
        self.port_info.n_params = self.port_params.len() as u32;
        self.monitor_port_info.change_mask =
            (spa::sys::SPA_PORT_CHANGE_MASK_PROPS | spa::sys::SPA_PORT_CHANGE_MASK_PARAMS) as u64;
...
unsafe extern "C" fn pipewire_bridge_exported_node_add_listener(
    ...
    unsafe {
        spa::sys::spa_hook_list_append(&mut node.hooks, listener, events.cast(), data.cast())
    };
    if !events.is_null() {
        unsafe {
...
unsafe extern "C" fn pipewire_bridge_exported_node_port_use_buffers(
    ...
    for index in 0..(n_buffers as usize) {
        ...
        if flags & spa::sys::SPA_NODE_BUFFERS_FLAG_ALLOC != 0 {
            ...
            let storage =
                node.owned_buffer_data[index].get_or_insert_with(|| vec![0u8; requested_size]);
...
unsafe extern "C" fn pipewire_bridge_exported_node_process(object: *mut c_void) -> i32 {
    ...
    if !metrics.first_process_logged {
        metrics.first_process_logged = true;
    }
    process_pipewire_bridge_chunk_metrics(&mut metrics, chunk, || {
...
unsafe fn for_each_pipewire_bridge_node_listener<F>(hooks: &mut spa::sys::spa_hook_list, mut f: F)
where
