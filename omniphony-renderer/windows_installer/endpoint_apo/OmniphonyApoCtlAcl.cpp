...
LSTATUS WriteDefaultMode(HKEY key) {
    ...
    return RegSetValueExW(
        key, kEfxModesValue, 0, REG_MULTI_SZ,
...
LSTATUS WriteStateDword(const std::wstring& endpointGuid, const wchar_t* name, DWORD value) {
    const std::wstring path = std::wstring(kStateBase) + endpointGuid;
    HKEY key = nullptr;
...
bool ReadStateDword(const std::wstring& endpointGuid, const wchar_t* name, DWORD& value) {
    return ReadRegDword(std::wstring(kStateBase) + endpointGuid, name, value);
}

void RemoveState(const std::wstring& endpointGuid) {
    const std::wstring path = std::wstring(kStateBase) + endpointGuid;
...
int Attach(const Endpoint& endpoint) {
    ...
    LSTATUS status = WriteStateDword(endpoint.guid, L"ModesExisted", modesExisted ? 1u : 0u);
    if (status != ERROR_SUCCESS) {
    ...
    if (status == ERROR_SUCCESS && !modesExisted) status = WriteDefaultMode(key);
    ...
    std::wstring verify;
    if (!ReadRegString(fx, kEfxValue, verify) || _wcsicmp(verify.c_str(), ours.c_str()) != 0) {
        std::wcerr << L"ERROR\tFX_VERIFY\n";
        return 6;
    }

    std::wcout << L"APO_ATTACHED\t" << endpoint.name << L"\t" << endpoint.guid
               << L"\t" << endpoint.id << L"\n";
...
int Detach(const Endpoint& endpoint) {
    ...
    const std::wstring ours = GuidText(kOmniphonyApoClsid);
    std::wstring existing;
    const bool oursAttached =
        ReadRegString(fx, kEfxValue, existing) && _wcsicmp(existing.c_str(), ours.c_str()) == 0;

    DWORD modesExisted = 1;
    const bool removeModes = ReadStateDword(endpoint.guid, L"ModesExisted", modesExisted) && modesExisted == 0;
    if (!oursAttached && !removeModes) {
        RemoveState(endpoint.guid);
        std::wcout << L"APO_DETACHED\t" << endpoint.name << L"\t" << endpoint.guid
                   << L"\t" << endpoint.id << L"\n";
        std::wcout << L"RESTART_AUDIO_REQUIRED\t1\n";
        return 0;
    }
