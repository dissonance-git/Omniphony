...
[CmdletBinding()]
param(
    [string]$EqualizerApoPath = ''
)

$ErrorActionPreference = 'Stop'
Set-StrictMode -Version Latest

$beginMarker = '# BEGIN OMNIPHONY PERSONAL BOOTSTRAP (identity only)'
...
foreach ($required in @($bridgeSource, $runtimeSource)) {
    if (!(Test-Path $required)) {
        throw "Bootstrap artifact is incomplete: missing $required"
    }
}
...
if (!(Test-Path $configPath)) {
    throw "Equalizer APO config.txt was not found at $configPath"
}
...
@"
# Omniphony personal bootstrap
# Stage 0 is deliberately bit-exact identity. If enabling this changes sound,
# stop and use uninstall-personal-bootstrap.ps1 before proceeding.
VSTPlugin: Library "$bridgeInstalled"
"@ | Set-Content -Path $snippetPath -Encoding UTF8

$configText = Get-Content -Raw -Path $configPath
$cleanText = (Remove-MarkedBlock $configText).TrimEnd("`r", "`n")
...
Write-Host ''
Write-Host 'Omniphony personal bootstrap installed.' -ForegroundColor Green
Write-Host "Equalizer APO: $eapoRoot"
Write-Host "Bridge:        $bridgeInstalled"
Write-Host "Config backup: $backupPath"
Write-Host ''
Write-Host 'This stage is intentionally inaudible: the Omniphony realtime ABI is exact identity.'
Write-Host 'If your FiiO endpoint was already enabled in Equalizer APO/HeSuVi, no endpoint change is needed.'
Write-Host 'If it was not, enable the FiiO render endpoint with Equalizer APO Configurator and reboot/restart the audio service as that tool requests.'
